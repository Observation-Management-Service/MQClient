...
def test_create_pub_queue(mock_pika: Any, queue_name: str) -> None:
    """Test creating pub queue."""
    q = rabbitmq.Backend().create_pub_queue("localhost", queue_name)
    assert q.queue == queue_name
    mock_pika.return_value.channel.assert_called()


def test_create_sub_queue(mock_pika: Any, queue_name: str) -> None:
    """Test creating sub queue."""
    q = rabbitmq.Backend().create_sub_queue("localhost", queue_name, prefetch=213)
    assert q.queue == queue_name
    assert q.prefetch == 213
    mock_pika.return_value.channel.assert_called()


def test_send_message(mock_pika: Any, queue_name: str) -> None:
...
def test_get_message(mock_pika: Any, queue_name: str) -> None:
    """Test getting message."""
    q = rabbitmq.Backend().create_sub_queue("localhost", queue_name)
...
def test_ack_message(mock_pika: Any, queue_name: str) -> None:
    """Test acking message."""
    q = rabbitmq.Backend().create_sub_queue("localhost", queue_name)
    q.ack_message(12)
...
def test_reject_message(mock_pika: Any, queue_name: str) -> None:
...
def test_message_generator_0(mock_pika: Any, queue_name: str) -> None:
...
def test_message_generator_1(mock_pika: Any, queue_name: str) -> None:
    """Test message generator."""
    q = rabbitmq.Backend().create_sub_queue("localhost", queue_name)
    fake_message = (MagicMock(delivery_tag=12), None, b'foo, bar')
...
def test_message_generator_2(mock_pika: Any, queue_name: str) -> None:
    """Test message generator."""
    q = rabbitmq.Backend().create_sub_queue("localhost", queue_name)
    fake_message = (MagicMock(delivery_tag=12), None, b'foo, bar')
...
def test_message_generator_upstream_error(mock_pika: Any, queue_name: str) -> None:
...
def test_message_generator_no_auto_ack(mock_pika: Any, queue_name: str) -> None:
    """Test message generator.
...
def test_message_generator_propagate_error(mock_pika: Any, queue_name: str) -> None:
    """Failure-test message generator.
...
def test_message_generator_suppress_error(mock_pika: Any, queue_name: str) -> None:
    """Failure-test message generator.
